...
# EDIT HERE variable addresses -> enter your public node addresses here (make sure the nodes are chronologically added (on time when first set up))
addresses = {"node_1":"0x_address_1",
             "node_2":"0x_address_2",
             "node_3":"0x_address_..."}
...
mining_start_datetime = datetime.strptime(json_data["claimedRewardCodes"][0]["claimTime"][:-5].replace("T"," "), "%Y-%m-%d %H:%M:%S")
...
def obtain_info():
    ...
    for key, value in addresses.items():
        ...
        try: 
            ...
            last_reward_per_node.append(str(int((datetime.utcnow() - node_reward_datetime).seconds  / 60)) + " minutes ago")
            ...
            elif ((datetime.utcnow() - node_reward_datetime).seconds > 4500 and (datetime.utcnow() - node_reward_datetime).seconds < 9000): 
    ...
    # average revenue per dag and hour, calculated from total revenue up to current date
    node_rev_day = round((((data_per_node[0] * coin_value) / (mining_days * 24 + mining_hours)) * 24 ) * len(addresses.keys()), 2)
    node_rev_hour = round((((data_per_node[0] * coin_value) / (mining_days * 24 + mining_hours))) * len(addresses.keys()), 2)
    ...
    node_est_rev_month = round((((data_per_node[0] * coin_value) / (mining_days * 24 + mining_hours)) * 732 ) * len(addresses.keys()), 2)
    node_est_rev_year = round((((data_per_node[0] * coin_value) / (mining_days * 24 + mining_hours)) * 8772 ) * len(addresses.keys()), 2)
    ...
    for index, address in enumerate(addresses):
        print(f'    Node {index + 1} | Gathered:      {data_per_node[index]} DATA / ${round(data_per_node[index] * coin_value, 2)}')
